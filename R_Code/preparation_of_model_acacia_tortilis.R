...
#read presence data
presence <- readRDS("data/combined_data/acacia_tortilis_combined.rds")
head(presence)
names(presence)
dim(presence)
#read bioclim data
...
x11()
pairs(sdmdata[,2:12], cex=0.1, fig=T)
#corrplot(cor(sdmdata[,1:19]), type = "lower", diag=F)

corr <- cor(na.omit(sdmdata[,2:19]), method = "spearman")
...
me <- maxent(bioclim_africa[[c("bio10", "bio18", "bio5", "bio6", "bio12", "bio13", "bio16", "bio2", "bio4", "bio1", "bio3", "bio14")]], p=presence_africa@coords, a=background)
